...
# Additional endpoint from simple API
class SimpleRouteRequest(BaseModel):
    origin: Tuple[float, float] = Field(..., description="(lat, lon) del origen")
    destination: Tuple[float, float] = Field(..., description="(lat, lon) del destino")
    weather_condition: str = Field("clear", description="Condición climática")
    hurry_factor: float = Field(50.0, ge=0, le=100, description="Factor de prisa")
...
@app.post("/v1/route/safe", response_model=SimpleRouteResponse)
async def get_safe_route_simple(request: SimpleRouteRequest, _=Depends(get_api_key)):
    """Simple route calculation endpoint."""
    if G_BASE is None:
        raise HTTPException(status_code=500, detail="Graph Engine Offline")

    try:
        G_local = engine.aplicar_formula_sandoval(G_BASE.copy(), weather_impact=request.weather_factor, hurry_factor=request.hurry_factor)
        ruta, n_orig, n_dest = engine.calcular_ruta_optima(G_local, request.origin, request.destination, criteria='final_impedance')
        distancia = nx.shortest_path_length(G_local, n_orig, n_dest, weight='length')
        explicacion = f"Análisis Sandoval: Ruta optimizada para {request.weather_condition}."
